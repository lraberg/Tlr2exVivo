...
files <- file.path(getwd(), paste(c("abundance","counts","length"),"rsem","tsv",sep="."))
...
#creating df for the scores
pca.bor.cond.scores <- as.data.frame(cbind( "NGI.ID"= rownames(rv.pca.bor.cond$x),rv.pca.bor.cond$x))

#merging score with metaData
pca.bor.cond.scores <- merge(pca.bor.cond.scores, metaData, by="NGI.ID")

#converting score to numeric for linear modeling later
pca.bor.cond.scores$PC1 <- as.numeric(pca.bor.cond.scores$PC1)

#pc1 boxplot
box.pc1.bor.cond.degs.all <-  ggplot(pca.bor.cond.scores) +geom_boxplot(aes(x=group, y=-PC1, group=group, fill=Condition))+
  scale_fill_manual(labels=c("*B. afzelii*", "Unstimulated"),values=c("#78A71F","#FFB419")) + 
  geom_point(aes(x=group, y=-PC1, group=groupSex, shape=Sex), position = position_jitterdodge(), size=2.25, alpha=1/2,colour = "black", fill = "white", stroke = 0.5) +
  scale_shape_manual(labels=c("F","M"), values=c(0,2)) + theme_bw() + scale_x_discrete(labels=c("c1/c1", "c1/c2","c2/c2","c1/c1", "c1/c2","c2/c2")) +
...
pc1.bor.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ Condition:Sex+ Genotype.linear:Sex + Condition:Sex:Genotype.linear+ (1|Vole), data=pca.bor.cond.scores)
car::Anova(pc1.bor.cond.lmer, type="3", test.statistic = "F")


pc1.bor.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ Condition:Sex+ Genotype.linear:Sex + (1|Vole), data=pca.bor.cond.scores)
car::Anova(pc1.bor.cond.lmer, type="3", test.statistic = "F")

pc1.bor.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ Condition:Sex+ (1|Vole), data=pca.bor.cond.scores)
car::Anova(pc1.bor.cond.lmer, type="3", test.statistic = "F")
...
#fetching score
pca.gas.cond.scores <- as.data.frame(cbind( "NGI.ID"= rownames(rv.pca.gas.cond$x),rv.pca.gas.cond$x))

#merging with metadata
pca.gas.cond.scores <- merge(pca.gas.cond.scores, metaData, by="NGI.ID")

#converting to numeric
pca.gas.cond.scores$PC1 <- as.numeric(pca.gas.cond.scores$PC1)

#pc1 boxplot
box.pc1.gas.cond.degs.all <-  ggplot(pca.gas.cond.scores) +geom_boxplot(aes(x=group, y=-PC1, group=group, fill=Condition))+
  scale_fill_manual(labels=c("*S.pyogenes*", "Unstimulated"),values=c("#71a3d7","#FFB419")) + 
  geom_point(aes(x=group, y=-PC1, group=groupSex, shape=Sex), position = position_jitterdodge(), size=2.25, alpha=1/2,colour = "black", fill = "white", stroke = 0.5) +
  scale_shape_manual(labels=c("F","M"), values=c(0,2)) + theme_bw() + scale_x_discrete(labels=c("c1/c1", "c1/c2","c2/c2","c1/c1", "c1/c2","c2/c2")) +
...
pdf("degs_gas_cond_boxAll.pdf", height = 6, width=8)
...
pc1.gas.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ Condition:Sex+ Genotype.linear:Sex + Condition:Sex:Genotype.linear+ (1|Vole), data=pca.gas.cond.scores)
car::Anova(pc1.gas.cond.lmer, type="3", test.statistic = "F")

pc1.gas.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ Condition:Sex+ Genotype.linear:Sex + (1|Vole), data=pca.gas.cond.scores)
car::Anova(pc1.gas.cond.lmer, type="3", test.statistic = "F")

pc1.gas.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ Condition:Sex+ (1|Vole), data=pca.gas.cond.scores)
car::Anova(pc1.gas.cond.lmer, type="3", test.statistic = "F")

pc1.gas.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex + Genotype.linear:Condition+ (1|Vole), data=pca.gas.cond.scores)
car::Anova(pc1.gas.cond.lmer, type="3", test.statistic = "F")

pc1.gas.cond.lmer <- lmer(PC1 ~ Condition  + Genotype.linear + Sex +(1|Vole), data=pca.gas.cond.scores)
car::Anova(pc1.gas.cond.lmer, type="3", test.statistic = "F")
...
##GO TERM analysis
#list of degs
geno.degs <- list("B. afzelii c1/c2 \n vs c1/c1"=rownames(res.geno.de.list$Borrelia.2VS1), 
                  "B. afzelii c2/c2 \n vs c1/c1"=rownames(res.geno.de.list$Borrelia.3VS1),
                  "B. afzelii c2/c2 \n vs c1/c2"=rownames(res.geno.de.list$Borrelia.3VS2),
                  "S. pyogenes c1/c2 \n vs c1/c1"=rownames(res.geno.de.list$GAS.2VS1),
                  "S. pyogenes c2/c2 \n vs c1/c1"=rownames(res.geno.de.list$GAS.3VS1),
                  "S. pyogenes c2/c2 \n vs c1/c2"=rownames(res.geno.de.list$GAS.3VS2),
                  "Unstimulated c1/c2 \n vs c1/c1"=rownames(res.geno.de.list$Unstimulated.2VS1),
                  "Unstimulated c2/c2 \n vs c1/c1"=rownames(res.geno.de.list$Unstimulated.3VS1),
                  "Unstimulated c2/c2 \n vs c1/c2"=rownames(res.geno.de.list$Unstimulated.3VS2)
                  )

#overlap between genotypes borrelia
venn.bor123 <- ggvenn(geno.degs[1:3], fill_color = c("#CC64DE","#1E88E5","#F0F2A5"), text_size = 6, set_name_size =6)
venn.gas123 <- ggvenn(geno.degs[4:6], fill_color = c("#CC64DE","#1E88E5","#F0F2A5"), text_size = 6, set_name_size = 6)
venn.uns123 <- ggvenn(geno.degs[7:9], fill_color = c("#CC64DE","#1E88E5","#F0F2A5"), text_size = 6, set_name_size = 6)

pdf("geno_venn.pdf", height=12, width=12)
ggarrange(venn.bor123, venn.gas123, venn.uns123, nrow=2, ncol=2)
dev.off()
...
pdf("genotype_GO.pdf", height = 10, width=12)
...
#Co-expression similarity and adjacency; picking power threshold 10
...
#estimating correlations between modules and trait and pvalues for correlation
...
module.filt <- 0:(nrow(module.table)-1)
module.filt <- module.filt[!(module.filt %in% c(0,7,20,23))]
               
boxPlot.allModule <- lapply(module.filt, function(x){
...
names(boxPlot.allModule) <- paste("Module",module.filt,sep=".")

goTerm.Module <- lapply(module.filt, function(x){
  vst.mod <- vst.dds[rownames(vst.dds) %in% moduleGeneMap$gene_id[moduleGeneMap$modules==x],]
  ...
  if(nrow(mod.go) > 0){
    ...
    dotplot(mod.go, showCategory=5, title=name, font.size=15)
...
pdf("allModule_pc1_boxplot.pdf", height = 20, width=18)
do.call(ggpubr::ggarrange, c(boxPlot.allModule, ncol = 4, nrow=5, common.legend = TRUE))
dev.off()

pdf("allModule_go.pdf", height =20, width=28)
do.call(ggpubr::ggarrange, c(goTerm.Module.plot, nrow =4, ncol=4))
dev.off()
...
key.geneDrivers.module9.bor.go.plot <- dotplot(key.geneDrivers.module9.bor.go, showCategory=10, title="Key module 9 genes in B. afzelii", font.size=18)
...
#making dataframe with all elements
mod2.pca.score.bor <- as.data.frame(cbind( "NGI.ID"= rownames(rv.mod2.pca.bor$x),rv.mod2.pca.bor$x))


#merging with metadata
mod2.pca.score.bor <- merge(mod2.pca.score.bor, metaData, by="NGI.ID")

#setting pc1 to numeric
mod2.pca.score.bor$PC1 <- as.numeric(mod2.pca.score.bor$PC1)

#making condition as factor
mod2.pca.score.bor$Condition <- as.factor(as.character(mod2.pca.score.bor$Condition))

#linear modelling for module 2 affecting borrelia; successively removing non-sign terms
lmer.mod2.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ Sex:Condition:Genotype.linear+ (1|Vole), data=mod2.pca.score.bor)
car::Anova(lmer.mod2.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod2.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+(1|Vole), data=mod2.pca.score.bor)
car::Anova(lmer.mod2.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod2.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Condition+(1|Vole), data=mod2.pca.score.bor)
car::Anova(lmer.mod2.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod2.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Sex:Condition+(1|Vole), data=mod2.pca.score.bor)
car::Anova(lmer.mod2.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod2.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + (1|Vole), data=mod2.pca.score.bor)
car::Anova(lmer.mod2.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod2.linear.pc1.bor <- lmer(PC1 ~ Condition + Sex + (1|Vole), data=mod2.pca.score.bor)
car::Anova(lmer.mod2.linear.pc1.bor, type="3", test.statistic="F")
...
#making dataframe with all elements
mod9.pca.score.bor <- as.data.frame(cbind( "NGI.ID"= rownames(rv.mod9.pca.bor$x),rv.mod9.pca.bor$x))


#merging with metadata
mod9.pca.score.bor <- merge(mod9.pca.score.bor, metaData, by="NGI.ID")

#setting pc1 to numeric
mod9.pca.score.bor$PC1 <- as.numeric(mod9.pca.score.bor$PC1)


box.pc1.bor.mod9.all <- ggplot(mod9.pca.score.bor) +geom_boxplot(aes(x=group, y=-PC1, group=group, fill=Condition))+
  scale_fill_manual(labels=c("*B. afzelii*", "Unstimulated"),values=c("#78A71F","#FFB419")) + 
  geom_point(aes(x=group, y=-PC1, group=groupSex, shape=Sex), position = position_jitterdodge(), size=2.25, alpha=1/2,colour = "black", fill = "white", stroke = 0.5) +
  scale_shape_manual(labels=c("F","M"), values=c(0,2)) + theme_bw() + scale_x_discrete(labels=c("c1/c1", "c1/c2","c2/c2","c1/c1", "c1/c2","c2/c2")) +
  theme (axis.text = element_text(size=18), axis.title = element_text(size=20), plot.tag = element_markdown(size=16), legend.text = element_markdown(size=18), 
         legend.title = element_text(size=18),legend.background = element_blank(), legend.position = "top", panel.grid.major = element_blank(), 
         panel.grid.minor = element_blank(), panel.border = element_rect(linewidth = 1, color="black")) +
  labs(x="Genotype", y="PC1") + guides(shape=guide_legend(override.aes = list(size=3)))



pdf("mod9_boxplot_bor.pdf", height = 8, width=8)
box.pc1.bor.mod9.all
dev.off()


#making condition as factor
mod9.pca.score.bor$Condition <- as.factor(as.character(mod9.pca.score.bor$Condition))

lmer.mod9.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ Sex:Condition:Genotype.linear+ (1|Vole), data=mod9.pca.score.bor)
car::Anova(lmer.mod9.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod9.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ (1|Vole), data=mod9.pca.score.bor)
car::Anova(lmer.mod9.linear.pc1.bor, type="3", test.statistic="F")

lmer.mod9.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Condition+ (1|Vole), data=mod9.pca.score.bor)
car::Anova(lmer.mod9.linear.pc1.bor, type="3", test.statistic="F")
...
#making dataframe with all elements
key.mod9.pca.score.bor <- as.data.frame(cbind( "NGI.ID"= rownames(rv.key.mod9.pca.bor$x),rv.key.mod9.pca.bor$x))


#merging with metadata
key.mod9.pca.score.bor <- merge(key.mod9.pca.score.bor, metaData, by="NGI.ID")

#setting pc1 to numeric
key.mod9.pca.score.bor$PC1 <- as.numeric(key.mod9.pca.score.bor$PC1)


#making condition as factor
key.mod9.pca.score.bor$Condition <- as.factor(as.character(key.mod9.pca.score.bor$Condition))

#modelling
lmer.key.mod9.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ Sex:Condition:Genotype.linear+ (1|Vole), data=key.mod9.pca.score.bor)
car::Anova(lmer.key.mod9.linear.pc1.bor, type="3", test.statistic="F")

lmer.key.mod9.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ (1|Vole), data=key.mod9.pca.score.bor)
car::Anova(lmer.key.mod9.linear.pc1.bor, type="3", test.statistic="F")

lmer.key.mod9.linear.pc1.bor <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Condition+ (1|Vole), data=key.mod9.pca.score.bor)
car::Anova(lmer.key.mod9.linear.pc1.bor, type="3", test.statistic="F")
...
#making dataframe with all elements
mod2.pca.score.gas <- as.data.frame(cbind( "NGI.ID"= rownames(rv.mod2.pca.gas$x),rv.mod2.pca.gas$x))

#merging with metadata
mod2.pca.score.gas <- merge(mod2.pca.score.gas, metaData, by="NGI.ID")

#setting pc1 to numeric
mod2.pca.score.gas$PC1 <- as.numeric(mod2.pca.score.gas$PC1)

#making conditoin as factor
mod2.pca.score.gas$Condition <- as.factor(as.character(mod2.pca.score.gas$Condition))

lmer.mod2.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ Sex:Condition:Genotype.linear+  (1|Vole), data=mod2.pca.score.gas)
car::Anova(lmer.mod2.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod2.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ (1|Vole), data=mod2.pca.score.gas)
car::Anova(lmer.mod2.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod2.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Condition+ (1|Vole), data=mod2.pca.score.gas)
car::Anova(lmer.mod2.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod2.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + (1|Vole), data=mod2.pca.score.gas)
car::Anova(lmer.mod2.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod2.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + (1|Vole), data=mod2.pca.score.gas)
car::Anova(lmer.mod2.linear.pc1.gas, type="3", test.statistic = "F")
...
#making dataframe with all elements
mod9.pca.score.gas <- as.data.frame(cbind( "NGI.ID"= rownames(rv.mod9.pca.gas$x),rv.mod9.pca.gas$x))

#merging with metadata
mod9.pca.score.gas <- merge(mod9.pca.score.gas, metaData, by="NGI.ID")

#setting pc1 to numeric
mod9.pca.score.gas$PC1 <- as.numeric(mod9.pca.score.gas$PC1)


#making condition as factor
mod9.pca.score.gas$Condition <- as.factor(as.character(mod9.pca.score.gas$Condition))

lmer.mod9.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ Sex:Condition:Genotype.linear+ (1|Vole), data=mod9.pca.score.gas)
car::Anova(lmer.mod9.linear.pc1.gas, type="3", test.statistic="F")

lmer.mod9.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ (1|Vole), data=mod9.pca.score.gas)
car::Anova(lmer.mod9.linear.pc1.gas, type="3", test.statistic="F")

lmer.mod9.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+  (1|Vole), data=mod9.pca.score.gas)
car::Anova(lmer.mod9.linear.pc1.gas, type="3", test.statistic="F")

lmer.mod9.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Sex:Genotype.linear+  (1|Vole), data=mod9.pca.score.gas)
car::Anova(lmer.mod9.linear.pc1.gas, type="3", test.statistic="F")
...
#making dataframe with all elements
mod15.pca.score.gas <- as.data.frame(cbind( "NGI.ID"= rownames(rv.mod15.pca.gas$x),rv.mod15.pca.gas$x))


#merging with metadata
mod15.pca.score.gas <- merge(mod15.pca.score.gas, metaData, by="NGI.ID")

#setting pc1 to numeric
mod15.pca.score.gas$PC1 <- as.numeric(mod15.pca.score.gas$PC1)

#making conditoin as factor
mod15.pca.score.gas$Condition <- as.factor(as.character(mod15.pca.score.gas$Condition))

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ Sex:Condition:Genotype.linear+  (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ Sex:Condition+ (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Condition:Genotype.linear + Sex:Genotype.linear+ (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + Sex:Genotype.linear+ (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ Sex + (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + Genotype.linear+ (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")

lmer.mod15.linear.pc1.gas <- lmer(PC1 ~ Condition + (1|Vole), data=mod15.pca.score.gas)
car::Anova(lmer.mod15.linear.pc1.gas, type="3", test.statistic = "F")
...
tlr2.violin <- ggplot(tlr2.vst, aes(x= Genotype, y=vst, fill=Condition)) + geom_violin(inherit.aes = T, trim = F, show.legend = F, alpha=0.5) + 
  theme_bw() + theme(axis.text = element_markdown(size=18), axis.title=element_markdown(size=20), strip.text.x = element_markdown(size=18), plot.tag = element_markdown(size=17, face="bold"), plot.title = element_markdown(size=20, face = "bold.italic",hjust=0.5),  legend.text = element_markdown(size=16), legend.position = "top", legend.title = element_text(size=17)) + 
  geom_boxplot(show.legend = F,width=0.1) + facet_grid(~Condition, labeller = labeller(Condition=c("Borrelia"="*B. afzelii*","GAS"="*S. pyogenes*" ,"Unstimulated"="Unstimulated"))) +
  scale_fill_manual(values=c("#78A71F","#71a3d7","#FFB419")) +
  scale_x_discrete(labels=c("c1/c1","c1/c2","c2/c2")) + labs(x="Genotype", y="Gene Expression (vst)") +
  geom_point(aes(x=Genotype, y=vst, group=genoSex, shape=Sex), position = position_jitterdodge(), size=2.25, alpha=1/2,colour = "black", fill = "white", stroke = 0.5) +
  scale_shape_manual(labels=c("F","M"), values=c(0,2))  + guides(shape=guide_legend(override.aes = list(size=3)))

pdf("tlr2_violin.pdf", height = 9, width=10.5)
tlr2.violin
dev.off()
...
tlr2.hap.boxplot <- ggplot(data=tlr2.afc.melt) + geom_boxplot(aes(x=Condition,y=aFC, fill=Condition), show.legend = F, width=0.5) + 
  ...
  geom_point(aes(x=Condition, y=aFC, group=paste(Condition, Sex), shape=Sex), position = position_jitterdodge(), size=2.25, alpha=1/2,colour = "black", fill = "white", stroke = 0.5) +
  scale_shape_manual(labels=c("F","M"), values=c(0,2)) + theme_bw() +
  theme (axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.text = element_markdown(size=18), axis.title = element_markdown(size=20), legend.text = element_markdown(size=18), legend.title = element_text(size=18),legend.background = element_blank(), legend.position = "right") +
  guides(shape=guide_legend(override.aes = list(size=3))) 

pdf("ase_tlr2.pdf",height = 9, width=9)
ggarrange(tlr2.hap.plot, NA, tlr2.hap.boxplot, nrow=3, common.legend= T, legend="right",align = "hv", heights = c(1,0.01,0.75))
dev.off()
